...
function.
-}
minmax :: (Num t, Ord t) => t -> t -> t -> t
minmax x y z =
    let maxima = max x (max y z)
        minima = min x (min y z)
    in maxima - minima
...
-}
strSum :: String -> Int
strSum str = recursiveStrSum (words str) where
    recursiveStrSum :: [String] -> Int
    recursiveStrSum numbers =
        if null numbers
            then 0
            else (read (head numbers) :: Int) + recursiveStrSum (tail numbers)
...
lowerAndGreater n list =
    ...
    where
        ...
        count fn x numbers
            | null numbers          = 0
            | fn x (head numbers)   = count fn x (tail numbers) + 1
            | otherwise             = count fn x (tail numbers)
